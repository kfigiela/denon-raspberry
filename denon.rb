...
#          if checksum == bytes[-1]
 #           got_packet packet[6...-1] # packet without header and checksum
  ...
  def on_button(id)
    case @status.source
    when :network then on_network_button(id)
    when :cd then on_cd_button(id)
    when :aux1 then on_analog1_button(id)
    when :aux2 then on_analog2_button(id)
    when :digital then on_digital_button(id)
    end
  end
