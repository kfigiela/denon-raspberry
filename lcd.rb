...
def word_wrap2(text, col_width=16)
   ...
   if tmp.length == 0
     ...
     [tmp[0].center(col_width), tmp[-1].center(col_width)]
   else
     [tmp[0].center(col_width)] + tmp[1...-1].map { |l| l.center(col_width) } + [tmp[-1].center(col_width)]
   end
end
...
class LCD
  def initialize(common)
    ...
    HD44780.init
    @counter = 0
    EM.add_periodic_timer(2) { refresh_screen; @counter += 1 }
    @line1 = @line2 = [""]
    @status = nil
  ...
  def refresh_screen
    line1_current = @line1[@counter % @line1.length].ljust(16)
    line2_current = @line2[@counter % @line2.length].ljust(16)

    if line1_current != @line1_current or line2_current != @line2_current then
      HD44780.init
      @line1_current = line1_current
      HD44780.puts line1_current, 0
      @line2_current = line2_current
      HD44780.puts line2_current, 1
    end
  end
end
